...
func (pool *TxPool) GetSuggestedFees(client *Client, baseFeeWei *big.Int, tipFeeWei *big.Int) (feeCap *big.Int, tipCap *big.Int, err error) {
	...
	if feeCap == nil || tipCap == nil {
		...
		if feeCap == nil {
			...
			//
			// Use baseFee * 1.25 + tipCap: just enough headroom to prevent the
			// oscillation (survives 1 full block of 12.5% baseFee increase plus
			// pool tracking staleness), but low enough that transactions start
			// becoming underpriced after 2 consecutive full blocks. This preserves
			// EIP-1559's self-regulating behavior when throughput exceeds the
			// 50% gas target — baseFee rises and pushes out old transactions,
			// naturally reducing effective inclusion toward the target.
			currentBaseFee := pool.GetCurrentBaseFee()
			if currentBaseFee != nil && currentBaseFee.Sign() > 0 {
				feeCap = new(big.Int).Mul(currentBaseFee, big.NewInt(5))
				feeCap.Div(feeCap, big.NewInt(4))
